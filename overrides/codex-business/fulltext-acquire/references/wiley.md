...
Use the bridge's documented PDF handling or a visible native download control;
keep session-bearing embedded URLs inside the selected browser.
...
2. If that control directly downloads a file, await only the new stable landing.
   ...
   bridge's documented PDF handling when available, or its native recovery
   through the visible wrapper/viewer Download control. Inspect that control
   using the current binding; do not assume a fixed wrapper action exists,
   click a hidden iframe, or construct `/doi/pdfdirect/`.
3. Follow the bridge's download completion contract. If an event is absent,
   times out, or supplies no documented file-saving/path API, accept only a
   newly landed stable file in the controlled directory and record
   `fallback_directory_increment`. An event alone is not file verification.
