...
Maintain `literature/FULLTEXT_MANIFEST.md` when requested, already used by the project, or required by a downstream research consumer. A standalone download needs source/version and verification evidence, without automatically creating the full manifest. The schema below applies when the full contract is selected.
