...
3. For a direct download, snapshot the approved landing directory immediately
   ...
   PDF tab without exposing its signed query, then use the bridge's documented
   PDF handling when available or the viewer's visible native Download
   control. Follow the bridge's native recovery and disk-completion rules;
   do not assume a fixed loaded-PDF action exists or use page JavaScript to
