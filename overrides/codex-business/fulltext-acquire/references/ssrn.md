...
Follow the bridge's download contract: snapshot the actual controlled landing directory, arm a download event when documented, and activate the SSRN PDF control. If the event is absent, times out, or has no file-saving/path API, use the shared narrow directory-increment fallback. Use the bridge's native download recovery if the route fails. Verify PDF integrity, title/authors, size, hash, and required provenance.
