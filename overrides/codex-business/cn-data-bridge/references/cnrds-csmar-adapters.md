...
8. **One Codex route** — every model selected inside Codex uses the current native Chrome binding through `browser-session-bridge`, which resolves Computer Use or an available legacy binding. Do not substitute another browser backend or Agent runtime.
...
- In the current CNRDS skin the final control can appear in accessibility state as a short private-use icon-font `StaticText`, not a button. When there is exactly one visible **压缩完成 + 未下载** row matching the dataset and one matching download icon, use the bridge's supported AX action or screenshot-grounded native click. Do not assume a fixed queue-icon API exists. Re-inspect ambiguous state before selecting a control.
