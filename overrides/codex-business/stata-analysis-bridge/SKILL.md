...
If Stata execution details matter, read `references/stata-analysis-guide.md`. Read `../shared-references/business-helper-resolution.md` before invoking the submit helper outside the ARIS repository.
...
Write or repair the do files needed for the requested specifications and diagnostics, using table shells or the supplied design. Relevant tasks include:
...
- Record Stata version and command dependencies. Include output hashes in `repro_lock` only when the project uses that contract.
