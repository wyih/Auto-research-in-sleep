...
This is the complete pipeline handoff template. Use it when the user requests the full contract or the consuming workflow requires it. For standalone analysis, use the focused fields in `SKILL.md`; do not fill unrelated blocks or describe a focused note as a completed pipeline card.
