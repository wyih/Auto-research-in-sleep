...
Reuse the project's layout. Create only files needed for the requested analysis; a full project may use:
...
Write or repair the scripts needed for the requested specifications and diagnostics, using table shells or the supplied design. Relevant tasks include:
...
Record scripts, logs, outputs, sample, specification, and findings in existing analysis notes. For a full analysis or a consumer requiring named artifacts, write:
...
When the user requests a Word results document or the current deliverable explicitly includes one:
...
- Check reported numbers against outputs; use `business-number-audit` when a separate audit is requested or required by the project's workflow.
- When a Word results document is part of the requested deliverable, route to `results-to-docx` after tidy coef export; never overwrite manuscript files for that step.
- Record R and key package versions. Include output hashes in `repro_lock` only when the project uses that contract.
