...
When the executor runs under **Kimi Code CLI**, the reviewer route changes
host, not honesty level. The normative contract lives in
`skills/skills-kimi/shared-references/reviewer-routing.md` (generated by
`tools/build_skills_kimi.py`); this section is the mainline summary.

- **Default: Kimi Code subagent.** Round 1 spawns a fresh reviewer subagent
  through the host's `Agent` tool (`kimi_subagent` contract); follow-up rounds
  resume it (`kimi_subagent_continue` with `resume: <saved agent id>`).
- **No effort/model knobs.** Kimi Code's `Agent` tool exposes no
  reasoning-effort or model parameter, so there is no tier table and no
  capability-fallback chain — delegate at the host's strongest reasoning
  configuration. If the `Agent` tool is unavailable or the spawn fails, emit
  `BLOCKED` / `REVIEW_UNAVAILABLE`; never substitute the executor's own
  judgment and never fabricate a provisional PASS.
- **Honesty:** the reviewer is the same model family as the executor, so every
  positive base verdict records `review_independence: same-family` /
  `acceptance_status: provisional` — identical in kind to the Codex base
  package.
- **Cross-family upgrade:** register the neutral `llm-chat` MCP server
  (`mcp-servers/llm-chat/`) in the Kimi Code configuration (MCP registry in
  `SETUP_GUIDE.md` §3.2, review-channel notes in `docs/KIMI_ADAPTATION.md`),
  then route through `mcp__llm-chat__review` / `mcp__llm-chat__review_reply`.
  It may record `acceptance_status: accepted` only when executor/reviewer
  families are known and different (`independence_verified`); otherwise it
  fails closed. `oracle-pro` (GPT family) is cross-family for a Kimi Code
  executor, subject to the same trace-binding rule.
