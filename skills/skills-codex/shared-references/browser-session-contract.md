...
|---|---|---|
| `session.attach` | Connect to the selected authorized browser runtime; an isolated-space adapter must select only the current run's space | Adapter identity, implementation, profile mode, and available tab/space context |
| `tab.list` | Enumerate open tabs without inspecting private storage | Titles/URLs needed for target selection only |
| `tab.open_or_claim` | Reuse a suitable tab or use the selected adapter's bounded same-profile blank-tab bootstrap before navigation | Active target tab and page/controller lease where the adapter requires one |
...
- A single-space persistent browser profile has at most one active controller across projects and processes. An isolated-space runtime may run projects concurrently only when every run owns a different Task Space; one Task Space still has at most one controller. Use a runtime-enforced lease or ownership boundary where available. Hold ownership through download landing/copy verification, then release it before local analysis or public-web work. Selected tabs, foreground state, and prose coordination files are not ownership proof. If isolated spaces share one filesystem download directory and lack event-backed saves, serialize only the final click-to-copy interval.
...
- A healthy browser transport with no suitable tab is a `tab.open_or_claim` bootstrap branch, not an adapter failure. Use only the selected adapter's bounded operation; never call raw DevTools `new_page`, CDP `/json/new`, AppleScript, or a second browser backend.
...
Each operation returns client runtime, adapter, implementation, MCP server (or `none`), profile mode, site, session reuse, isolated-space use, `session_recovery: not_needed|dismiss_refresh_restored|dismiss_refresh_still_logged_out`, `saved_login_submitted: true|false`, login-state category, operation, artifact path, expected format, size, hash, verification status, and blocker. `saved_login_submitted` records only that the authorized submit-only transition occurred; it never records form values, account identity, or password-manager state. It omits all authentication material, account identifiers, and published Task Space identifiers.
