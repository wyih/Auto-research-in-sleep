...
description: Reuse the user's authorized Chrome session from Codex for authenticated web navigation and verified downloads. Use for CNKI, SSRN, ScienceDirect, Wiley, CSMAR, CNRDS, library portals, subscription pages, protected fields, interactive exports, or entitled downloads that require existing Chrome state. Route public pages to web-fetcher and open research-paper retrieval to fulltext-acquire first.
---

# Browser Session Bridge

Perform one authenticated browser operation through Codex's native Chrome capability, land the requested artifact, and return verifiable evidence. The selected model does not change the browser route: any model running inside Codex uses the same Codex plugin.
...
- [references/codex-chrome.md](references/codex-chrome.md) for the native Chrome binding.
...
Record the selected value as `browser_required_reason` before acquiring Chrome.
...
2. Load `chrome:control-chrome` and use its native Chrome binding. Do not substitute another browser backend, standalone Playwright, or another profile.
...
6. If the user has authorized submission and Chrome has already populated a normal login form, click its login control once without reading or typing field values. Pause for the user when fields are empty, MFA or account choice is required, the attempt errors, or a hard CAPTCHA is active.
7. Execute only the caller's site recipe and minimal requested export or download.
8. Snapshot the approved landing directory and arm download handling before the final click. On event timeout, use only the contract's narrow new-and-stabilized-file fallback.
...
10. Recheck caller-specific identity, columns, periods, grain, or content. Release the Chrome binding after verification.

## Receipt

Return `client_runtime: codex`, `adapter: codex_native_chrome`, `mcp_server: native`, `implementation: codex_chrome`, and `profile_mode: user_chrome`, followed by the site, operation, browser-required reason, session reuse, login-state category, artifact path, expected format, size, SHA-256, generic verification result, caller-specific verification result, download completion mode, and blocker if any.
