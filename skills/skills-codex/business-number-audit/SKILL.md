...
Read `references/number-audit-checklist.md` when the audit is high-stakes or the paper is near submission. Read `../shared-references/business-helper-resolution.md` before invoking the checker outside the ARIS repository.
...
```bash
python3 "$NUMBER_AUDIT_SKILL_DIR/scripts/verify_numbers.py" \
  --project . \
