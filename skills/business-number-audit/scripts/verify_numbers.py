...
def should_skip(raw: str, value: float) -> bool:
    ...
    if value == 0:
        return True
    return value == int(value) and 1 <= value <= 20 and "." not in raw and "," not in raw
...
def number_matches(value: float, references: set[float], tolerance: float) -> bool:
    ...
    for ref in references:
        if ref == 0:
            continue
        for candidate in (ref, ref * 100, ref / 100, ref * 1000, ref / 1000):
            denominator = abs(candidate) if candidate else 1.0
            if abs(value - candidate) / denominator <= tolerance:
                return True
    return False
...
def build_report(
    ...
    for entry in paper_numbers:
        destination = matched if number_matches(float(entry["value"]), references, tolerance) else unmatched
    ...
    if unmatched:
        lines.extend(("| Number | Line | Context |", "|--------|------|---------|"))
...
def main() -> int:
    ...
    return 1 if unmatched_count and not args.allow_unmatched else 0
