...
- `browser-session-bridge` before any portal interaction; it owns the authenticated browser-session contract
...
Follow `references/cnrds-csmar-adapters.md` as the site recipe and invoke `browser-session-bridge`. The site recipe owns portal/module/filter intent; the bridge owns host-native browser execution, authenticated-session reuse, download completion, and its redacted receipt.
...
Each portal requires its own browser-session receipt. A CSMAR pass does not prove CNRDS, and a CNRDS pass does not prove CSMAR.
