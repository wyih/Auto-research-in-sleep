...
> **Kimi Code assurance:** every base semantic audit is same-family provisional.
...
- the host reviewer model reviews the plan for completeness
...
2. **Reviewer pushes back.** Spawn a FRESH reviewer agent (host's strongest reasoning configuration)
...
- the host reviewer model reviews figure quality and captions
...
- the host reviewer model reviews each section for quality
...
if paper contains \begin{theorem} or \begin{lemma} or \begin{proof}:
    ...
    This invokes host-strongest subagent to:
...
**Round 1:** host-strongest subagent reviews the full paper → identifies CRITICAL/MAJOR/MINOR issues → Claude Code implements fixes → recompile → save `main_round1.pdf`

**Round 2:** host-strongest subagent re-reviews the recompiled draft cold (fresh review — no fix summaries, no conversation carry-over) → identifies remaining issues → Claude Code implements fixes → recompile → save `main_round2.pdf`
...
if paper/references.bib (or paper.bib) exists and contains entries cited from sec/*.tex:
    ...
    Fresh Kimi Code subagent reviewer (`kimi_subagent`, same-family provisional) with web/DBLP/arXiv lookup
    verifies each entry:
...
### Phase 5.9: Integrity Forensics (optional here — see Kimi Code-native note)
...
gate + append-only obligations). **In a Kimi Code-native session this is OFF by
default**: upstream ships no Kimi Code-native pack, so only its deterministic-only
...
for a Kimi Code executor — informational, since this gate only raises flags and
...
> clone (e.g. `~/Desktop/Auto-claude-code-research-in-sleep`). In a Kimi Code
> project install, derive it from `.aris/installed-skills-kimi.txt`:
> `ARIS_REPO="$(awk -F '\t' '$1=="repo_root"{print $2; exit}' .aris/installed-skills-kimi.txt)"`.
...
Each sub-audit runs in a **fresh Kimi Code thread** (never a continuation reply,
...
```bash
# Resolve the audit verifier (Policy A — gate; Kimi Code-side chain).
...
Kimi Code stop hook if their local Kimi Code runtime supports hooks:
