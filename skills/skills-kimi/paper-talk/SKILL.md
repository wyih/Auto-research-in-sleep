...
6. **Fresh-context independence.** Per-page Kimi Code calls in `/slides-polish` use fresh `kimi_subagent` calls (no `kimi_subagent_continue`) and are same-family provisional in the base package.
7. **Anonymity fail-closed.** If any audit (or any Kimi Code fix proposal) would replace a placeholder with a real title / count / URL, the workflow halts and surfaces the proposal for human review. See `../shared-references/experiment-integrity.md`.
...
10. **host-strongest reasoning delegation** is invariant across all `effort` levels for any Kimi Code call invoked by sub-skills.
...
2. **Check prerequisites**:
   ...
   - the Kimi Code Agent tool availability.
...
`/slides-polish` emits a `BLOCKED` verdict because a Kimi Code fix proposal
...
- **Open warnings**: any unresolved Kimi Code polish notes the user should review by hand.
...
Host-strongest reasoning delegation is invariant.
...
per-page Kimi Code polish pass (Phase 3) is the single most expensive but
