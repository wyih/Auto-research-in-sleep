...
- **EXA_FETCHER** — canonical name `exa_search.py`, resolved per
  ...
  (Kimi Code-side chain: `$ARIS_REPO/tools/` → `tools/` → `~/.kimi-code/skills/exa-search/`).
...
```bash
# Resolve $EXA_FETCHER via the canonical strict-safe Kimi Code chain.
...
[ -z "$EXA_FETCHER" ] && {
  echo "ERROR: exa_search.py not resolved at \$ARIS_REPO/tools/, tools/, or ~/.kimi-code/skills/exa-search/." >&2
  echo "       Fix: rerun tools/install_aris_kimi.sh, export ARIS_REPO, or copy the helper to ~/.kimi-code/skills/exa-search/." >&2
...
```
exa_search.py not found. Run install_aris_kimi.sh, set ARIS_REPO to your ARIS repo root, or install/copy the helper into the project/global Kimi Code skill path; then install exa-py:
pip install exa-py
```
