...
description: "Generate publication-quality AI illustrations for academic papers using Gemini image generation. Creates architecture diagrams, method illustrations with Kimi Code-supervised iterative refinement loop. Use when user says \"生成图表\", \"画架构图\", \"AI绘图\", \"paper illustration\", \"generate diagram\", or needs visual figures for papers."
---

# Paper Illustration: Multi-Stage Kimi Code-Supervised Figure Generation

Generate publication-quality illustrations using a **multi-stage workflow** with **Kimi Code as the STRICT supervisor/reviewer**.
...
│       ▼                                                                  │
│   ┌─────────────┐                                                        │
│   │   Kimi Code    │ ◄─── Step 1: Parse request, create initial prompt     │
│   │  (Planner)  │                                                        │
│   └──────┬──────┘                                                        │
│          │                                                               │
│          ▼                                                               │
...
│          ▼                                                               │
│   ┌─────────────┐                                                        │
│   │   Kimi Code    │ ◄─── Step 5: STRICT visual review + SCORE (1-10)      │
...
### Step 1: Kimi Code Plans the Figure (YOU ARE HERE)

**CRITICAL: Kimi Code must first analyze the user's request and create a detailed prompt.**
...
**Prompt Template for Kimi Code CLI to generate:**
...
**Kimi Code sends the initial prompt to Gemini (gemini-3-pro) for layout optimization.**
...
# The initial prompt from Kimi Code
INITIAL_PROMPT='[Kimi Code fills in the detailed prompt here]'
...
**Kimi Code sends the optimized layout to Gemini for CVPR/NeurIPS style verification.**
...
**Kimi Code sends the optimized, style-verified specification to Paperbanana for rendering.**
...
try:
    ...
    iteration = 1  # Kimi Code increments this each iteration
...
### Step 5: Kimi Code STRICT Visual Review & Scoring (MANDATORY)

**Kimi Code MUST read the generated image and perform a STRICT review:**
...
```markdown
## Kimi Code's STRICT Review of Figure v{N}
...
**Kimi Code generates TARGETED improvement prompt with EXACT issues:**
...
7. **Kimi Code is the STRICT boss** — Accept only excellence, not "good enough"
...
11. **USE MULTI-STAGE WORKFLOW** — Kimi Code → Gemini Layout → Gemini Style → Paperbanana → Kimi Code Review
...
| Stage | Model | Purpose |
|-------|-------|---------|
| Step 1 | Kimi Code | Parse request, create initial prompt |
...
| Step 5 | Kimi Code | STRICT visual review and scoring |
