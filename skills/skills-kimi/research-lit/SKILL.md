...
- **REVIEWER_BACKEND = `kimi-subagent`** — Default reviewer route for optional literature synthesis cross-checks. Use `--reviewer: oracle-pro` only when explicitly requested; if Oracle is unavailable, warn and continue with the default subagent reviewer or local synthesis.
...
| 5 | **Semantic Scholar API** | `semantic-scholar` | `$S2_FETCHER` resolves (canonical name `semantic_scholar_fetch.py`, per integration-contract §2 Kimi Code chain) | Published venue papers (IEEE, ACM, Springer) with structured metadata: citation counts, venue info, TLDR. **Only runs when explicitly requested** |
...
| 9 | **OpenAlex** | `openalex` | `$OPENALEX_FETCHER` resolves (canonical name `openalex_fetch.py`, per integration-contract §2 Kimi Code chain) and Python `requests` is importable | Open citation graph with institutional affiliations, funding data, and broad work metadata. **Only runs when explicitly requested** |
...
Retrieval and extraction are breadth-bound. Use fresh `kimi_subagent` shards when
...
verification remains deterministic; optional Kimi Code synthesis review is
...
Resolve `$ARXIV_FETCHER` via the canonical strict-safe Kimi Code chain
...
[ -z "$ARXIV_FETCHER" ] && [ -f ~/.kimi-code/skills/arxiv/arxiv_fetch.py ] && ARXIV_FETCHER="$HOME/.kimi-code/skills/arxiv/arxiv_fetch.py"
...
[ -z "$S2_FETCHER" ] && [ -f ~/.kimi-code/skills/semantic-scholar/semantic_scholar_fetch.py ] && S2_FETCHER="$HOME/.kimi-code/skills/semantic-scholar/semantic_scholar_fetch.py"
...
[ -z "$DEEPXIV_FETCHER" ] && [ -f ~/.kimi-code/skills/deepxiv/deepxiv_fetch.py ] && DEEPXIV_FETCHER="$HOME/.kimi-code/skills/deepxiv/deepxiv_fetch.py"
...
[ -z "$EXA_FETCHER" ] && [ -f ~/.kimi-code/skills/exa-search/exa_search.py ] && EXA_FETCHER="$HOME/.kimi-code/skills/exa-search/exa_search.py"
...
[ -z "$OPENALEX_FETCHER" ] && [ -f ~/.kimi-code/skills/openalex/openalex_fetch.py ] && OPENALEX_FETCHER="$HOME/.kimi-code/skills/openalex/openalex_fetch.py"
