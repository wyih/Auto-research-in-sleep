...
- `REVIEWER_MODEL = the host reviewer model` — External examiner for claim quality review
...
Start the examiner review with a dedicated Kimi Code reviewer subagent at the host's strongest reasoning configuration:
...
4. Re-submit to the same examiner subagent via Agent-tool `resume` using the saved agent id
...
kimi_subagent_continue:
  ...
  resume: [saved reviewer id from Step 5]
  prompt: |
...
- If reviewer delegation is unavailable in the current Kimi Code host, stop and ask the user to enable Kimi Code subagent support before continuing the examiner loop.
