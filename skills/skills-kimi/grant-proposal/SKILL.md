...
**What this does:**
- host-strongest subagent acts as a grant review panelist (not a paper reviewer)
...
- Timeline: [summary]
- Reviewer feedback: [key points from the host reviewer model]
...
**What this does:**
- host-strongest subagent acts as a grant review panelist
...
If `/research-review` is invoked (preferred), it handles the external review internally. If you run the reviewer directly, use `kimi_subagent` for Round 1 and `kimi_subagent_continue` for follow-up rounds.
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via Agent-tool `resume`.
...
- Timeline: [N] years
- Review score: [summary from the host reviewer model]
...
- **Feishu notifications are optional.** If `~/.kimi-code/feishu.json` exists, send `checkpoint` at each phase transition and `pipeline_done` at final output. If absent, skip silently.
...
| `reviewer model` | the host reviewer model | Kimi Code review model | → reviewer agent |
