...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Kimi Code/the host reviewer model): Review for fidelity, specificity, contribution quality, and frontier leverage
  -> Phase 3 (Local step): Anchor check + simplicity check -> revise method -> rewrite full proposal
  -> Phase 4 (Kimi Code, same agent): Re-evaluate revised proposal
...
Send the full proposal to the host reviewer model for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
kimi_subagent:
  ...
  prompt: |
    ...
    **OVERALL SCORE** (1-10): Weighted toward Problem Fidelity, Method Specificity, Contribution Quality, and Frontier Leverage.
    ...
    paragraph. A fresh Kimi Code positive review is same-family provisional.
...
Send the revised proposal back to the host reviewer model in the **same agent**:

```text
kimi_subagent_continue:
  # Kimi Code Agent tool — resume the saved reviewer subagent
  resume: [saved from Phase 2]
...
[Full verbatim response from the host reviewer model]
