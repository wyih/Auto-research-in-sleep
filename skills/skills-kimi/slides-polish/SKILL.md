---
name: slides-polish
description: "Per-page Kimi Code review + targeted python-pptx / Beamer fixes for academic talk slides. Use AFTER /paper-slides (or any externally generated PPTX/Beamer) when the deck looks 'mostly OK' but the user wants a final pass that aligns visual weight with a reference, bumps PPTX fonts to projector-readable size, kills italic style leaks, fixes text-frame overflow, and catches per-slide layout drift. Trigger phrases: \"polish slides\", \"slides 排版不对\", \"PPTX 字体太小\", \"和 Beamer 比一下\", \"per-page review\", \"和 Kimi 一页一页过\"."
...
# Slides Polish: Per-Page Kimi Code Review + Targeted Layout Fixes

Polish a generated slide deck — Beamer (`.tex` + `.pdf`) and/or PPTX — by
running **per-page Kimi Code review** against a reference visual and applying
...
- **CONTEXT_POLICY = `fresh`** — Each per-page review uses a **fresh** Kimi Code reviewer call (`kimi_subagent`, never `kimi_subagent_continue`). See `../shared-references/reviewer-independence.md`. This prevents the reviewer from anchoring on prior fixes.
...
- **PDF inspection**: `pdfinfo` and either `pdftoppm` (poppler, preferred) or `mutool draw` (mupdf) for rendering slides to PNG. Required so the per-page Kimi Code call sees actual slide pixels, not text extraction alone. Render command: `pdftoppm -r 150 -png <pdf> <out-stem>` (or `mutool draw -o <out-stem>-%d.png -r 150 <pdf>`).
...
- If `pdftoppm`/`mutool` missing → ask user to install, do not proceed (visual review without rendered pages produces low-confidence Kimi Code feedback).
...
`.aris/slides-polish/<deck-stem>/`. Per-call Kimi Code traces also follow the
...
6. **Render PDF → PNG**: `pdftoppm -r 150 <pdf> .aris/slides-polish/<stem>/png/page` produces one PNG per slide; passed to Kimi Code during per-page review.
7. **Triage pass**: a single fresh Kimi Code call sweeps all N slides comparing PPTX-PDF (or Beamer PDF) against the reference. Output: per-slide verdict matrix.
...
Kimi Code call. Apply the returned fix immediately (subject to `INTERACTIVE`),
recompile or save, move to next slide.

**Per-page loop, not batch.** Empirically: per-page Kimi Code calls converge in
1-2 polish rounds where single-pass batch review never converges.

#### Per-page Kimi Code prompt template
...
After Kimi Code returns, call `apply_fix(slide_index, fix_block)` which:
...
Each per-page Kimi Code call returns specific shape resize commands; apply via
...
```
Slide  1 cover  | bumped wordmark 80→86; subtitle 36→40; line spacing 0.9        | Kimi Code per-page review
...
4. **No content edits.** No new claims, numbers, citations, URLs, author names, affiliations, or experiment results. No equation or figure-content changes. No paraphrasing of body text — only style/typography/box edits. If a Kimi Code fix proposal would change content, the skill stops and reports it.
5. **No slide reordering, addition, or deletion** unless the user passes an explicit flag (`— add-slide-K-after-J`, `— drop-slide-K`).
6. **Fresh-context independence**: per-page Kimi Code calls are fresh `kimi_subagent` calls, not `kimi_subagent_continue`. Reviewer never sees prior fix lists; base verdicts are same-family provisional.
7. **Anonymity placeholders fail closed.** If a Kimi Code fix proposes filling in a real title, count, or URL where a placeholder was, the skill rejects it and surfaces the proposal for human review. See `experiment-integrity.md`.
8. **Page numbers stay ≤ 16pt.** Why-RF discipline; never bump them.
9. **Host-strongest reasoning delegation** is invariant across all `effort` levels.
...
After each per-page Kimi Code call, save the trace following
...
- Uses the same `kimi_subagent` MCP infrastructure as
...
- Deck has fewer than 5 slides — per-page review overhead is not worth it;
  hand-edit.
- The user explicitly says "no Kimi Code" — this skill is Kimi Code-driven by design.
...
Kimi Code review with concrete python-pptx / `.tex` fix snippets converges in
