...
- **Fresh review at the artifact boundary.** Academic-template HTML is reviewed by a fresh Kimi Code `kimi_subagent`. In the base mirror this is same-family and the result is provisional; an overlay may provide cross-family accepted review. Dashboard HTML skips review by default but accepts `--review`.
...
Resolve `$RENDER_HTML` with the Kimi Code hybrid chain (managed project install, in-repo checkout, ARIS repo fallback, then copied global Kimi Code skill install). This is **Policy A — skill-local gate**:

```bash
RENDER_HTML=""
cd "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" || exit 1
if [ -z "${ARIS_REPO:-}" ] && [ -f .aris/installed-skills-kimi.txt ]; then
    ARIS_REPO=$(awk -F'\t' '$1=="repo_root"{print $2; exit}' .aris/installed-skills-kimi.txt 2>/dev/null) || true
fi
...
[ -z "$RENDER_HTML" ] && [ -f ~/.kimi-code/skills/render-html/scripts/render_html.py ] && RENDER_HTML="$HOME/.kimi-code/skills/render-html/scripts/render_html.py"
[ -z "$RENDER_HTML" ] && {
  echo "ERROR: render_html.py not resolved at .agents/skills/render-html/scripts/, skills/render-html/scripts/, \$ARIS_REPO/skills/render-html/scripts/, or ~/.kimi-code/skills/render-html/scripts/." >&2
  echo "       /render-html cannot produce HTML output. Fix: rerun install_aris_kimi.sh, export ARIS_REPO, or copy the render-html skill into ~/.kimi-code/skills/." >&2
  exit 1
}
```
...
Kimi Code subagent with no author context before being claimed as reviewed. The base
...
**If `should_review` is true**, fire a fresh `kimi_subagent` call (NEVER `kimi_subagent_continue`; delegate at the host's strongest reasoning configuration per `../shared-references/reviewer-routing.md`) with the prompt below. The reviewer reads the source MD + generated HTML directly; it does **not** see this skill's intermediate state.
...
**If the Agent tool is not available** (e.g., the host session does not expose subagents): emit `verdict: REVIEW_UNAVAILABLE` to the sidecar, do not fabricate `PASS`, and tell the user the HTML was generated but **not** independently reviewed. The user can manually invoke `/research-review` on the source MD or re-run in a session with the Agent tool available.
...
For deck / poster / Xiaohongshu card / tweet card / data report style outputs, point users to **[html-anything](https://github.com/nexu-io/html-anything)** (Apache-2.0, 3000⭐ at time of writing). It ships 75 SKILL.md templates across 9 surfaces and detects 8 coding-agent CLIs (including Claude Code, Kimi Code, Copilot). ARIS does *not* depend on it — `/render-html` covers ARIS-native artifacts; html-anything is the recommended path for richer publishing surfaces.
...
- **academic-template HTML is a reviewed artifact**, not raw output. Base Kimi Code review is fresh but same-family provisional; never call it cross-model acceptance. `--no-review` exists for fast iteration but should not be the way you ship.
