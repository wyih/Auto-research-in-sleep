...
Resolve the repo from the Kimi Code manifest, then the helper:
...
- **Reviewer context isolation applies.** When the wiki is read by reviewer skills, pass file paths only; base Kimi Code review is same-family provisional.
