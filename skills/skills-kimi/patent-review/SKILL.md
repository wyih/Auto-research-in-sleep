...
# Patent Examiner Review via a Kimi Code subagent (host's strongest reasoning configuration)
...
- `REVIEWER_MODEL = the host reviewer model` — Model used via a Kimi Code subagent
- `REVIEW_ROUNDS = 2` — Number of review rounds
- `EXAMINER_PERSONA = "patent-examiner"` — the host reviewer model persona
...
kimi_subagent_continue:
  ...
  resume: [saved reviewer id from Round 1]
  prompt: |
