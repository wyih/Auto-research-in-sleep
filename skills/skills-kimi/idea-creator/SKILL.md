...
- **REVIEWER_BACKEND = `kimi-subagent`** — Default: Kimi Code reviewer subagent through the kimi_subagent / kimi_subagent_continue contract. Use `--reviewer: oracle-pro` only when explicitly requested; if Oracle is unavailable, warn and fall back to the default subagent reviewer.
...
Idea generation is breadth-bound, so use one fresh `kimi_subagent` shard per
...
or write shared files. The final Kimi Code jury sees the full deduped set and records
...
Resolve the wiki helper using the Kimi Code-side canonical chain (see
...
[ -z "$WIKI_SCRIPT" ] && [ -n "$ARIS_HOME" ] && [ -f "$ARIS_HOME/.kimi-code/skills/research-wiki/research_wiki.py" ] && WIKI_SCRIPT="$ARIS_HOME/.kimi-code/skills/research-wiki/research_wiki.py"
THREAT_SCANNER=""
...
Use a secondary Kimi Code subagent for divergent thinking:
...
Save a Review Tracing record for this `kimi_subagent` call following `../shared-references/review-tracing.md`, including the landscape summary, prompt summary, raw idea list path, reviewer route, and saved agent id.
...
> job of the Phase-4 fresh reviewer (same-family provisional in the base package). Dropping
...
1. **Novelty check**: Use the `/novelty-check` workflow (multi-source search + the host reviewer model cross-verification) for each idea
...
2. **Critical review**: Resume the same reviewer subagent via Agent-tool `resume`:
   ...
   kimi_subagent_continue:
     ...
     resume: [saved reviewer id from the earlier idea review]
     prompt: |
...
3. **Combine rankings**: Merge your assessment with the host reviewer model's ranking. Select top 2-3 ideas for pilot experiments.
...
After each `kimi_subagent` or `kimi_subagent_continue` reviewer call, save the trace following `../shared-references/review-tracing.md`. Include the reviewer route, saved agent id, prompt summary, raw output path, selected ideas, and rejected ideas.
