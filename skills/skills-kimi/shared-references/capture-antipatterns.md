...
> **Kimi Code mirror adaptation (normative).** Resolve helpers through the Kimi Code
> manifest and `$ARIS_REPO/tools/`. A clean same-family Kimi Code review may drive a
...
| **negative tool-capability claim** | "the review CLI can't handle long files", "gemini is broken", "don't use oracle" | the workaround, or "needs flag X" — never "tool can't do Y" |
...
- **Mechanical** (deterministic, `tools/capture_filter.py`): the unambiguous
  ...
  nouns (the host CLI / gemini / oracle / the reviewer / the MCP / the CLI …).
...
**classified reviewer route** before it is trusted. Base Kimi Code review is
