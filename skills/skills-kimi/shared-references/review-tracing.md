...
Trace every Kimi Code reviewer call that serves a critique, scoring, claim-verification, experiment-audit, or patch-gating function.

This includes:

- `kimi_subagent` reviewer calls
- `kimi_subagent_continue` reviewer continuations
...
A Kimi Code-side SKILL must NOT hard-code `tools/save_trace.sh`; instead
...
```json
{
  ...
  "tool": "kimi_subagent",
  "model": "kimi-code",
  "reasoning_depth": "host-strongest",
...
For a `llm-chat` cross-family route write `review_independence: cross-family` and
