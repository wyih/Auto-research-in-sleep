...
> **Kimi Code mirror adaptation (normative).** The `— composed:` and `— standalone`
> interface is unchanged. Kimi Code subagents return their unique content to the
> parent; only the parent writes shared deliverables. Resolve trace helpers via
> the Kimi Code manifest and `$ARIS_REPO/tools/`.
