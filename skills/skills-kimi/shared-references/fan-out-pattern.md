...
> **Kimi Code mirror adaptation (normative).** Kimi Code uses `kimi_subagent` for Tier-2
...
> candidates. A Kimi Code jury is same-family and records provisional; a
> `llm-chat` cross-family route or deterministic verifier records accepted.
...
base Kimi Code review remains provisional, while overlays and deterministic checks
...
it is Kimi Code/same-family/provisional; overlay or deterministic routes are accepted.
...
| Who runs it | Same-family subagents (Kimi Code shards) | Fresh Kimi Code (provisional) or overlay/deterministic route (accepted) |
...
primitive at all (`/kill-argument` runs two sequential fresh subagent
...
Kimi Code is provisional, while overlay/deterministic routes are accepted.
...
| **Tier 3** | Sequential fallback — the same N shards run one-by-one, each in a **fresh context** (context reset between shards) | Any runtime, including Kimi Code CLI / bare Claude Code with no Agent tool |
...
Deduplication runs on the merged union, **on the executor (Kimi Code),
...
(fresh Kimi Code / overlay / deterministic verifier) can be rate-limited and
...
```
fan-out (N shards) → merge union → mechanical dedup (Kimi Code, SAFE) → CLASSIFIED JURY
...
not a harness feature. It runs **two** fresh Kimi Code `kimi_subagent` threads
in series — Thread 1 writes the strongest 200-word rejection memo; Thread
2 (independent, no ``resume` continuation`) decomposes that memo into 3-7 atomic
...
jury here is same-family/provisional in the base package — both threads are
the Kimi Code reviewer, and **the skill code computes the
final verdict from per-point counts; the reviewer thread is forbidden from
...
**jury** is the already-existing Phase-4 fresh-Kimi Code devil's-advocate
pass: it is provisional in the base package and accepted only through an overlay
...
- **`reviewer-routing.md`** — jury backend selection. The default fresh Kimi Code
...
- **`acceptance-gate.md`** — when self-judgment is allowed. Self-judging
  ...
  base Kimi Code review and accepted only with an overlay/deterministic route. A
...
4. **A single classified jury step** (per `reviewer-routing.md` +
   `reviewer-independence.md`) — base Kimi Code is provisional; overlay or a
...
- **Tier-3** (sequential fallback) spawns nothing — e.g. `/kill-argument`
  runs its two passes as fresh Kimi Code subagent reviewer threads, no Agent tool.
...
**sub-agent**" in several skills refers to a fresh Kimi Code subagent reviewer (provisional
in the base package), not the Agent tool, and never implied a real grant need.
