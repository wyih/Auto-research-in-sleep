...
> **Kimi Code mirror adaptation (normative).** A Kimi Code-authored change reviewed by a
> fresh Kimi Code subagent uses `provenance.py stamp-provisional`. The receipt preserves
...
- **A provenance record cannot be same-family.** `stamp()` calls
  ...
  the reviewer are the same model family — e.g. `claude`+`sonnet`, or `kimi`+`kimi-code`,
  or the trap `gpt*`+`oracle-pro` (oracle routes to a GPT-Pro tier, so it is the
...
- **A provisional receipt may be same-family but grants no authorization.**
  ...
  `is_auto_curatable()` is false. This is the Kimi Code-only review path.
...
```json
{
  ...
  "verdict_id": "kimi_agent_abc123",
  "content_hash": "<sha256 of the artifact>",
  "stamped_at": "2026-05-30T00:00:00Z"
}
```

- `verdict_id` is the **traceable** acquittal — the reviewer thread id, the oracle
...
- **meta-optimize** — when an auto-patch lands (Step 6), `stamp()` the changed
  ...
  `reviewer_model` = the subagent/oracle reviewer that scored it (Step 4). The stamp
