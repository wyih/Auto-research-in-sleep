...
> **Kimi Code mirror adaptation (normative).** External cadence remains fire-control
> only. The default fresh Kimi Code subagent reviewer may produce a provisional stop/continue
> decision, while only a cross-family overlay or deterministic gate records
> accepted. Replace mainline the Kimi Code Agent tool examples with fresh `kimi_subagent` calls;
...
assigns it to — fresh Kimi Code quality/correctness verdicts are provisional;
...
- **Thread discontinuity.** ARIS's multi-round review skills carry state
  across rounds in the reviewer's own thread: ``resume` continuation` reuses the
...
| Acceptance gate | Machine-checkable existence/completion (safe same-model) | Quality/correctness (base Kimi Code = provisional; overlay/deterministic = accepted) |
...
- `/auto-review-loop` — already loops internally; reviewer carries
  round-to-round memory in one `threadId` (``resume` continuation`)
...
- `/auto-paper-improvement-loop` — review → fix → recompile loop with its own
  round structure and a fresh-reviewer bias guard each round (no ``resume` continuation`)
- `/research-review` — produces a traced review verdict (provisional in base Kimi Code)
