...
> **Kimi Code mirror adaptation (normative).** In this package the executor is
> the current Kimi Code main agent and the default reviewer is a fresh
> `kimi_subagent` subagent. That route is same-family: it may drive revisions and terminate a loop,
> but its positive verdict is recorded as `acceptance_status: provisional` and
> never as cross-family `accepted`. An `llm-chat` cross-family route or a deterministic
...
agents ran — decides whether a base Kimi Code review is provisional or an overlay /
...
mistaken for formal acceptance. The loop body can be all Kimi Code; the **gate**
...
or the review satisfied **as accepted assurance**. A base Kimi Code verdict is a
...
- ✅ the reviewer **was invoked** (a reviewer subagent returned, a JSON verdict file exists)
...
A judgment of *merit, correctness, or sufficiency*. A fresh base Kimi Code reviewer
...
model family through the `llm-chat` cross-family route, or a deterministic verifier,
...
> **No, it needs taste / correctness / domain judgment → Type-B** (base Kimi Code
...
contains 'ready'"* evaluated each round — the fresh base Kimi Code reviewer
...
STOP when "the paper is submission-ready"
  ├─ A: all 3 audits were invoked and emitted JSON   → Kimi Code self-checks
  ├─ A: verify_paper_audits.sh exit code == 0         → external process, Kimi Code reads it
...
4. **For every Type-B gate:** route it per `reviewer-routing.md`. The default
   fresh `kimi_subagent` review is `same-family` and therefore
   `provisional`; use a `llm-chat` cross-family route or deterministic verifier when
...
5. **State the provenance in the SKILL.** One line: "STOP gate = Type-B,
   base Kimi Code = provisional; overlay/deterministic route = accepted." A reviewer
...
quality decision to accepted, the loop is self-acquitting.** A base Kimi Code
...
| `/auto-review-loop` | score >= 6 AND verdict "ready", per round | B | fresh **Kimi Code** score & verdict | ⚠ provisional in base |
| `/auto-paper-improvement-loop` | "review satisfied" (2 rounds) | B | fresh **Kimi Code** review | ⚠ provisional in base |
| `/result-to-claim` | `claim_supported ∈ {yes,partial,no}` + `integrity_status` | B | fresh **Kimi Code** result judgment | ⚠ provisional in base |
| `/kill-argument` | rejection memo → defense, residual issues | B | two fresh **Kimi Code** threads | ⚠ provisional in base |
| `/proof-checker` | each gap closed, per round | B | fresh **Kimi Code** re-review | ⚠ provisional in base |
| `/experiment-audit` | integrity verdict (fake GT, normalization fraud) | B | fresh **Kimi Code** audit | ⚠ provisional in base |
| `/paper-claim-audit` | every number matches result files | B | fresh **Kimi Code** reviewer | ⚠ provisional in base |
| `/citation-audit` | every entry real & in-context | B | fresh **Kimi Code** reviewer | ⚠ provisional in base |
...
> siblings.) The fresh Kimi Code score+verdict is a Type-B review with
...
- **Every quality/correctness loop records its reviewer route.** Base Kimi Code
...
unsafe); the assurance class does not get silently upgraded — base Kimi Code stays
...
2. **Every Type-B gate records its reviewer route** per `reviewer-routing.md`;
   ...
   it. Base Kimi Code is provisional; overlay/deterministic routes are accepted.
...
- `reviewer-routing.md` — where Type-B gates send their verdict (kimi-subagent
