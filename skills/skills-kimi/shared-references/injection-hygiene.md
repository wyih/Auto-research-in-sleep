...
> **Kimi Code mirror adaptation (normative).** Resolve `threat_scan.py` from the
> installed Kimi Code manifest and `$ARIS_REPO/tools/`; do not assume project-local
> `tools/`. Scanner results are deterministic gates. Any semantic review by a
> fresh Kimi Code subagent remains same-family and provisional.
...
layer 2  semantic review         fresh Kimi Code = provisional; overlay/deterministic = accepted
```
...
- **research-wiki** (`tools/research_wiki.py`): edge `evidence` is quarantined
  ...
  scan-and-banner.)
- **`/idea-creator` query-pack load**: both the main skill and Kimi Code mirror scan
