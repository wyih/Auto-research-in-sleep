...
> **Kimi Code mirror adaptation (normative).** Start Kimi Code runs with
> `run_state.py start ... --executor kimi-code`. A fresh same-family Kimi Code reviewer
...
| **`provisional`** | a fresh same-family Kimi Code reviewer returned a positive verdict | **`mark-provisional` only** — requires executor, reviewer, verdict id, and a completed phase | terminal for resume ONLY when the run was started with `--provisional-advances` (the Kimi Code-native default recommendation); never equivalent to accepted |
...
**Resume walks forward to the first phase that is NOT terminal** ({`accepted`, `skipped`}; plus `provisional` only when the run's `policy.provisional_advances` is true — Kimi Code-native runs start with `--provisional-advances`, mainline runs keep cross-family-only advance) — never
...
Only:
- a **cross-family reviewer** verdict (`llm-chat` cross-family route, per
...
The **executor (Kimi Code) must never call `accept` on its own self-report.** Marking
your own phase done is fine (`set done`); acquitting it is not. `accept` records
the `reviewer` and REFUSES a known same-family pair (for a Kimi Code executor that
means `kimi*`/`moonshot*` reviewers — a `claude*` or `gemini*` reviewer is exactly
...
```
python3 tools/run_state.py start  <root> <run_id> --phases "W1,W1.5,W2,W3" --executor kimi-code --provisional-advances
python3 tools/run_state.py set    <root> <run_id> W1 done --artifact idea-stage/IDEA_REPORT.md
python3 "$RUN_STATE" start <root> <run_id> --phases W1,W1.5,W2 --executor kimi-code --provisional-advances
python3 "$RUN_STATE" mark-provisional <root> <run_id> W1 --verdict-id agent:019e... --reviewer kimi-code
...
3. **At the phase's gate:** run the phase's reviewer route. A fresh base Kimi Code
...
- `acceptance-gate.md` — the source rule (`done` = execution-completeness, safe
  same-model; fresh Kimi Code quality review = provisional; `accepted` =
