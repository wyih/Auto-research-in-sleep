...
On the Kimi Code side the helper may be at:

- `$ARIS_REPO/tools/<helper>` — env var or auto-resolved from `.aris/installed-skills-kimi.txt`
- `<project>/tools/<helper>` — manual copy or running from inside the ARIS repo
- `~/.kimi-code/skills/<skill-name>/<helper>` — Kimi Code global install layout
...
[ -z "$HELPER" ] && [ -f ~/.kimi-code/skills/<skill-name>/<helper> ] && HELPER="$HOME/.kimi-code/skills/<skill-name>/<helper>"
```
...
`install_aris_kimi.sh`), update scripts (`smart_update.sh`,
`install_aris_kimi.sh --reconcile`), manual setup (`overleaf_setup.sh`),
...
| `forensics_gate.py` (in `/integrity-forensics`, `/paper-writing` Phase 5.9/6.0, `/resubmit-pipeline`) | A (gate) | Typed policy gate + append-only obligations ledger for the Anti-Autoresearch launcher (deterministic slice in Kimi Code-native sessions); when forensics is in play an unresolved helper blocks the Final Report / Overleaf push (never improvise the gate). `fresh` is the one-command downstream preflight; exit code is the source of truth |
