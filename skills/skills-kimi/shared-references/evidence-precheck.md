...
> **Kimi Code mirror adaptation (normative).** Resolve `evidence_check.py` through
> `.aris/installed-skills-kimi.txt` and `$ARIS_REPO/tools/`. The deterministic
> existence check may be accepted; the fresh Kimi Code semantic review that follows
> is same-family and therefore provisional. Only a `llm-chat` cross-family route or a
...
Kimi Code mirror that call is same-family/provisional; an overlay or deterministic
...
stage 2  semantic review           fresh Kimi Code (provisional) / overlay or deterministic (accepted)
...
spares the reviewer call on fabricated evidence.
...
- **`/result-to-claim`** Step 1.5: parse each claim's cited `(value, source)`,
  run the batch pre-check, and **before the reviewer judgment** mark any claim whose
  evidence is `path_missing` / `value_not_found` as **unsupported — evidence not
  found**, and pass the per-claim pre-check status into the reviewer prompt so the
