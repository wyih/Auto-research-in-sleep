...
The skill reads `~/.kimi-code/feishu.json`. If this file does not exist, **all Feishu functionality is disabled** — skills behave exactly as before.
...
```bash
cat ~/.kimi-code/feishu.json 2>/dev/null
```
...
Check if `~/.kimi-code/feishu.json` exists and mode is not "off":
