...
> **Kimi Code assurance:** base Kimi Code proof judgments are
...
- **REVIEWER_BACKEND = `kimi-subagent`** — Default: fresh Kimi Code reviewer subagent via the Agent tool (host's strongest reasoning configuration). Override with `— reviewer: oracle-pro` for GPT-5.5 Pro via Oracle MCP, or register `llm-chat` for a cross-family accepted route. See `shared-references/reviewer-routing.md`.
...
`kimi_subagent` shards, with a sequential fresh-context fallback. Each shard
returns `{"shard_id": ..., "entries": [{"payload": ..., "dedup_key":
"<theorem-or-obligation-id>"}]}` and must not declare the proof valid. The
parent mechanically merges obligations; the fresh Kimi Code review that evaluates
...
### Phase 1: First Review (Kimi Code host-strongest subagent)
...
### Phase 3: Re-Review (Kimi Code host-strongest subagent)

Launch a fresh reviewer agent for the next review round. Do not use `kimi_subagent_continue` here; proof-checker keeps each round independent. Request the same mandatory checklist.
...
For any fix that resolved a FATAL or CRITICAL issue, submit the **fixed section alone** (without showing the previous critique) to a **fresh Kimi Code thread**:
...
Resolve the helper via the Kimi Code-side chain (skip cleanly if unavailable; the audit is
already complete):
```
...
- **Kimi Code executor analyzes and implements; a fresh Kimi Code subagent reviewer provides adversarial review.** Base review remains same-family/provisional.
...
- **Fresh reviewer agents**: Save each returned `agent_id` for traceability, but launch a new `kimi_subagent` for each review round. Do not use `kimi_subagent_continue` across proof-checker rounds.
...
```json
{
  ...
  "thread_id":        "<reviewer subagent or llm-chat thread id>",
  "executor_model":   "kimi-code",
  "executor_family":  "openai",
  "reviewer_model":   "the host reviewer model",
  "reviewer_family":  "openai",
  "review_independence": "same-family",
  "acceptance_status": "provisional",
  "reviewer_reasoning": "host-strongest",
...
Every invocation uses a fresh reviewer agent. Never use `kimi_subagent_continue` across
