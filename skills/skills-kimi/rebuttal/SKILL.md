...
- **REVIEWER_BACKEND = `kimi-subagent`** — Default: Kimi Code subagent stress tester. Use `--reviewer: oracle-pro` only when explicitly requested; if Oracle is unavailable, warn and fall back to the default subagent reviewer. See `../shared-references/reviewer-routing.md`.
- **MAX_INTERNAL_DRAFT_ROUNDS = 2** — draft → lint → revise.
- **MAX_STRESS_TEST_ROUNDS = 1** — One the Kimi Code Agent tool critique round.
...
### Phase 6: Kimi Code Reviewer Stress Test
...
6. Use the Kimi Code Agent tool reply for continuity if useful
...
After each `kimi_subagent` or `kimi_subagent_continue` reviewer call, save the trace following `../shared-references/review-tracing.md`. Write files directly to `.aris/traces/rebuttal/<date>_run<NN>/`. Respect the `--- trace:` parameter when present (default: `full`).
