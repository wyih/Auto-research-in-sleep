...
description: "Generate publication-quality academic illustrations through a local Codex app-server bridge that uses its native image generation. This is a separate experimental alternative to `paper-illustration`, intended for Claude Code users who want a GPT-image-style renderer without modifying the original skill."
argument-hint: "[description-or-method-file]"
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, mcp__image2__generate, mcp__image2__generate_start, mcp__image2__generate_status, Agent
---
...
│          ▼                                                               │
│   ┌─────────────┐                                                        │
│   │Claude/Kimi Code │ ◄─── Step 2: Optimize layout description               │
...
│          ▼                                                               │
│   ┌─────────────┐                                                        │
│   │Claude/Kimi Code │ ◄─── Step 3: CVPR/NeurIPS style verification           │
...
- **OPTIONAL_TEXT_CRITIC = `kimi_subagent`** — Optional text-only second opinion for layout/style checks
...
- **IMAGE2_HELPER** — canonical name `paper_illustration_image2.py`, resolved
  ...
  [ -z "$IMAGE2_HELPER" ] && [ -f ~/.kimi-code/skills/paper-illustration-image2/scripts/paper_illustration_image2.py ] && IMAGE2_HELPER="$HOME/.kimi-code/skills/paper-illustration-image2/scripts/paper_illustration_image2.py"
  [ -z "$IMAGE2_HELPER" ] && {
    echo "ERROR: paper_illustration_image2.py not resolved at .agents/skills/, \$ARIS_REPO/skills/, \$ARIS_REPO/tools/, tools/, or ~/.kimi-code/skills/." >&2
    echo "       /paper-illustration-image2 cannot proceed. Fix: rerun install_aris_kimi.sh, export ARIS_REPO, or copy the canonical skill into ~/.kimi-code/skills/." >&2
    exit 1
  }
  ```
...
📋 paper-illustration-image2 integration checklist:
   ...
   [ ] 3. Render via mcp__image2__generate_start + generate_status
...
If `kimi_subagent` is available, you may ask it for a short second-opinion
layout critique here, but Claude should still complete this step even without
Kimi Code.

Use Kimi Code layout critique for:
...
If `kimi_subagent` is available, you may ask it for a short text-only
style audit, but do not block on it.

## Step 4: Generate Through the Bridge

Call `mcp__image2__generate_start` with:
...
Then call `mcp__image2__generate_status` with bounded waits until:
...
| Step 2 | Claude (+ optional Kimi Code critique) | Refine layout, grouping, spacing, and arrow routing |
| Step 3 | Claude (+ optional Kimi Code critique) | Verify academic visual style before rendering |
| Step 4 | `mcp__image2__generate_start` + `generate_status` | Native raster image generation through Codex app-server |
