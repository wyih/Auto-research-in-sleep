...
continue to resolve. Kimi Code-side install layouts that previously
copied the canonical into `~/.kimi-code/skills/figure-spec/figure_renderer.py`
must now place it at `~/.kimi-code/skills/figure-spec/scripts/figure_renderer.py`
to match the new layout (re-run `install_aris_kimi.sh` to pick up
the new symlink target).

Resolve `$FIGURE_RENDERER` via the Kimi Code-side hybrid chain (layer 0
...
# Layer 4: Kimi Code-side skill-local install (`install_aris_kimi.sh` may place it here).
[ -z "$FIGURE_RENDERER" ] && [ -f ~/.kimi-code/skills/figure-spec/scripts/figure_renderer.py ] && FIGURE_RENDERER="$HOME/.kimi-code/skills/figure-spec/scripts/figure_renderer.py"
[ -z "$FIGURE_RENDERER" ] && [ -f ~/.kimi-code/skills/figure-spec/figure_renderer.py ] && FIGURE_RENDERER="$HOME/.kimi-code/skills/figure-spec/figure_renderer.py"  # pre-Phase-3.1 layout

[ -n "$FIGURE_RENDERER" ] || {
  echo "ERROR: figure_renderer.py not found at any of: \$ARIS_REPO/skills/figure-spec/scripts/, \$ARIS_REPO/tools/, tools/, ~/.kimi-code/skills/figure-spec/scripts/, ~/.kimi-code/skills/figure-spec/. Set ARIS_REPO, rerun install_aris_kimi.sh, or copy the canonical \$ARIS_REPO/skills/figure-spec/scripts/figure_renderer.py next to this skill." >&2
  exit 1
}
...
### Step 5: Iterate with Kimi Code Review (Optional, for High-Stakes Figures)

For paper architecture figures, invoke fresh-agent review (same-family provisional in the base package):
