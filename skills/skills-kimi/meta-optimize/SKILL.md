...
1. **Logging must be active.** Kimi Code mirror installs do not create Claude Code hooks. Provide `.aris/meta/events.jsonl` from a Kimi Code-compatible event logger, an external wrapper, or a manually exported trace log before running this skill.
...
if [ ! -f "$EVENTS_FILE" ]; then
    ...
    echo "Enable Kimi Code-compatible logging first: create .aris/meta/events.jsonl from your Kimi Code wrapper, external event logger, or exported trace log."
    exit 1
fi
...
— there is no equivalent `kimi` skill-selection probe yet, so for a Kimi Code
...
- **Anti-self-poisoning screen:** resolve `$ARIS_REPO` from
  `.aris/installed-skills-kimi.txt`, then run
...
Send each patch to host-strongest subagent for adversarial review:
...
{"ts":"...","session":"...","event":"kimi_subagent","tool":"kimi_subagent","input_summary":"review..."}
