...
Run proof work as a local-first pipeline. Kimi Code first attempts the proof, checks its correctness, and edits it for clarity and economy. Escalate the remaining hard obligation to GPT Pro.

Default escalation is manual: maintain the sources locally and give the user an exact browser-ready prompt. Invoking this skill does not authorize Kimi Code to operate a browser, upload files, or spend API credit. An optional external `call-gpt-pro` skill may be used only when it is installed and the user explicitly asks Kimi Code to perform the GPT Pro call for the current run.
...
Note on assurance families: for Kimi Code CLI, GPT Pro is the SAME model family as the
...
local-proof.md       # Kimi Code's proof attempt or isolated blocker
...
proof-ledger.md      # run state and provenance, optional
...
Treat an existing run, `next*.md`, `redo*.md`, or continuation artifact as a project continuation. First read the prior `final.md`, `audit.md`, `local-proof.md`, `proof-ledger.md`, `source-manifest.md`, `handoff.md`, and any next/redo/continuation files that exist. Use `gpt-pro-output.md` only as raw evidence unless its audit accepts the relevant claims.
...
Use these labels in `proof-ledger.md`, `audit.md`, or `handoff.md`:
...
4. Audit correctness locally.
   ...
   - Treat optional external or DeepSeek review as additional evidence, not a substitute for Kimi Code CLI's own audit, and do not trigger a paid or remote reviewer without authorization.
...
7. Dispatch only with explicit authorization and an installed route.
   - A request such as "use GPT Pro" does not by itself authorize Kimi Code to operate the browser or spend API credit; keep the manual route.
   - Switch to Kimi Code execution only when the user explicitly asks Kimi Code to call or operate GPT Pro for this run and a compatible `call-gpt-pro` skill is installed.
...
8. Ingest, audit, and edit the returned answer.
   - Save user-pasted or Kimi Code-retrieved text as `gpt-pro-output.md`.
   ...
   - If a central gap remains, mark `NEEDS_GPT_PRO_REDO` and prepare a focused manual redo prompt first. Dispatch the redo through Kimi Code only after new explicit authorization.
...
7. If the DeepSeek route is unavailable, mark `DEEPSEEK_REVIEW_BLOCKED`.
   ...
   `local-kimi-fallback`; it does not satisfy an independent cross-family
   acceptance gate.

DeepSeek may identify or propose a repair. Kimi Code validates each finding
against local sources and may downgrade an unverified issue to a candidate or
mark it disputed with evidence — but Kimi Code must never overturn an external
...
3. Make `browser-prompt.md` self-contained with the exact target, assumptions, definitions, requested output, and source filenames GPT Pro will see. Do not include local absolute paths, route bookkeeping, or instructions meant only for Kimi Code CLI.
...
Record nontrivial repairs in `audit.md` or `proof-ledger.md`. Perform substantive clarity and notation editing in `final.md`, after the correctness audit, rather than rewriting the raw output.
