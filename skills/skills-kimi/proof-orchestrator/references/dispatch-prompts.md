...
the user explicitly asks Kimi Code to do so for the current run.
...
Requirements:
- Read the prior final.md, audit.md, local-proof.md, proof-ledger.md,
...
```
## Explicit Kimi Code Dispatch Prompt

Use this template only after the user explicitly asks Kimi Code to perform the
current GPT Pro call:

```text
The user has explicitly authorized Kimi Code to dispatch this prepared GPT Pro
...
redo prompt. Do not dispatch it through Kimi Code without new explicit user
authorization.
```
...
- Do not let Kimi Code dispatch the redo unless the user explicitly asks it to do
  so for this turn.
```
