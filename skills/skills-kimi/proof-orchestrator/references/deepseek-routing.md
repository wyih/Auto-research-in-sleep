...
review branch in `proof-orchestrator`. Kimi Code remains the controller: gather
...
evidence or as cross-family review — for Kimi Code CLI this matters doubly, since a
...
`local-executor-fallback` (or `local-kimi-fallback` in the Kimi Code mirror)
