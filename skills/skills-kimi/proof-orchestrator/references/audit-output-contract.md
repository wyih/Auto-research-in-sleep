...
Reviewer backend: llm-chat-deepseek | local-executor-fallback | local-kimi-fallback
...
```json
{
  ...
  "reviewer_backend": "llm-chat-deepseek | local-executor-fallback | local-kimi-fallback",
...
they match; `none` for `local-executor-fallback` / `local-kimi-fallback`.
