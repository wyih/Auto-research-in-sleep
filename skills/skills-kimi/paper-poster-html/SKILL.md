...
fresh-agent review loop (same-family provisional in the base package), and the anti-patch-loop fix vocabulary.
...
- **SKILL_SCRIPTS** — helpers and templates are single-owner and ship inside the
  ...
  if [ -z "$SKILL_HOME" ] && [ -z "${ARIS_REPO:-}" ] && [ -f .aris/installed-skills-kimi.txt ]; then
  ...
  [ -z "$SKILL_HOME" ] && [ -d "$HOME/.kimi-code/skills/paper-poster-html/scripts" ] && SKILL_HOME="$HOME/.kimi-code/skills/paper-poster-html"
  if [ -z "$SKILL_HOME" ]; then
    echo "ERROR: paper-poster-html scripts not resolved. Re-run the ARIS Kimi Code install." >&2
  fi
...
- **REVIEWER_MODEL = `kimi-subagent`** — the host's strongest model, used via a fresh Kimi Code subagent (Agent tool; the host exposes no model/effort override parameter, so none is pinned).
  review call** (a new `kimi_subagent:` every time; never reuse a reviewer agent across
  review boundaries).
...
mandatory GAP paragraph. A fresh Kimi Code review may drive another layout round but
