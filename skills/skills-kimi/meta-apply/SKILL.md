---
name: meta-apply
description: "Privileged applier that LANDS meta-optimize / corpus-audit patches the user approved, with a fresh landing review and human approval. Base Kimi Code review is same-family provisional. Use when the user says \"meta apply\", \"/meta-apply\", \"land the staged patches\", \"应用优化\", after a /meta-optimize run."
...
> **Kimi Code assurance:** a base landing review records
...
2. **Fresh landing review PASS, obtained now.** Spawn a fresh `the host reviewer model`
...
3. **Record the review class honestly.** Base Kimi Code review is same-family and
   ...
   `llm-chat` cross-family route or deterministic verifier uses strict `stamp` and may
   record accepted. See
...
Resolve `provenance.py` through the Kimi Code manifest:
...
3. **Stamp provenance** on the changed file. Base Kimi Code uses:
   ```bash
...
- **Never promote provisional to accepted.** Base Kimi Code always uses
