...
### Step 3 — Fresh-agent math/code review (Kimi Code host-strongest subagent, same-family provisional)

Invoke the Kimi Code Agent tool with a fresh subagent at the host's strongest reasoning configuration. Do not reuse prior reviewer context.
...
For each FAIL issue, edit the MD. Then re-invoke Kimi Code with a **fresh `kimi_subagent` call** (never continue with `kimi_subagent_continue`). Stop when verdict = PASS or WARN with no FAIL items.
...
`render_html.py` runs its own 13-check review automatically. If that FAILs, fix the MD (often a table-pipe or callout-list issue the math/code reviewer missed) and re-render. Note that `render_html.py` itself writes `<slug>_tutorial.review.json` for the render-stage audit.
...
|---|---|
| Executor/reviewer family | Kimi Code drafts; fresh the host reviewer model reviews (math/code stage and render stage), same-family provisional |
| Fresh agent per reviewer call | Step 3 + render's own gate both use fresh `kimi_subagent` calls, not `kimi_subagent_continue` |
...
Extracted from the two pilot tutorials (Attention + Flow Matching, May 2026). Both passed fresh-agent review; that review is same-family provisional in the base Kimi Code mirror. The attention tutorial required 3 rounds and caught a table-pipe collision plus a callout-list collision.
