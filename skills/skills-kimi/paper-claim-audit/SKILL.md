...
> **Kimi Code assurance:** write `review_independence: same-family` and
> `acceptance_status: provisional` into base audit JSON. A fresh Kimi Code PASS may
...
### Step 1: Collect Files (Executor — Kimi Code)
...
### Step 2: Fresh Reviewer Audit (the host reviewer model — NEW thread, no reply)
...
### Step 3: Write Report (Executor — Kimi Code)
...
**Date**: [today]
**Auditor**: host-strongest subagent (fresh zero-context thread)
...
Uses **full review gate** (audit-class artifact; base Kimi Code review is fresh same-family provisional). Output: `paper/PAPER_CLAIM_AUDIT.html` with embedded source SHA256 + `.review.json` sidecar.

**Non-blocking**: if `/render-html` fails (helper missing, secondary Kimi Code subagent unavailable, file write error), log the failure and treat the audit as complete — the JSON + MD verdict files are canonical; the HTML view is a human-reader convenience.
...
- **Review class.** Base Kimi Code reviewer is same-family provisional; only an overlay may record cross-family accepted.
...
`kimi_subagent_continue`. Do not accept prior audit outputs (PROOF_AUDIT, CITATION_AUDIT,
