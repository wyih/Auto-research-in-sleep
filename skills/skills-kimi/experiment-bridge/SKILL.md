...
- **CODE_REVIEW = true** — Secondary Kimi Code reviewer at the host's strongest reasoning configuration reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip.
...
- **RESCUE_ON_FAILURE = true** — If sanity or deployment fails, run a Kimi Code-native rescue / second opinion review before abandoning the experiment plan.
...
Before deploying, send the experiment code to a secondary Kimi Code reviewer at the host's strongest reasoning configuration:
...
If the same sanity failure repeats, trigger a second opinion: summarize the plan, code diff, command, logs, backend, and failure, then ask a fresh Kimi Code subagent reviewer agent for a rescue diagnosis. Apply only concrete fixes grounded in the logs.
