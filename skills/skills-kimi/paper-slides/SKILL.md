...
### Phase 5: the Kimi Code Agent tool Review

Send the slide outline + selected LaTeX frames to host-strongest subagent:
...
> If reviewer delegation is unavailable in the current Kimi Code host, stop and ask the user to enable Kimi Code subagent support before continuing Phase 6.
...
- Files:
  ...
  ├── SLIDES_REVIEW.md      # the host reviewer model review feedback
...
- **Feishu notifications are optional.** If `~/.kimi-code/feishu.json` exists, send notifications. If absent, skip.
