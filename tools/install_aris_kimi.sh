#!/usr/bin/env bash
# install_aris_kimi.sh -- ARIS skill installation for Kimi Code CLI.
#
# Kimi Code discovers skills from <project>/.agents/skills/<name>/SKILL.md
# (project scope) and ~/.kimi-code/skills/<name>/SKILL.md (user scope) --
# the same SKILL.md format as the Codex release surface. This installer
# symlinks the Kimi-native package (generated by tools/build_skills_kimi.py):
#   <project>/.agents/skills/<skill-name> -> <aris-repo>/skills/skills-kimi/<skill-name>
#
# Coexistence with the Codex line: skills-kimi and skills-codex have DIVERGED
# (different reviewer-backend contracts) while .agents/skills/ is a discovery
# path shared by both CLIs -- so the two lines can never co-manage one project.
# When a selected name is already managed by install_aris_codex.sh (listed in
# .aris/installed-skills-codex.txt), the plan hard-fails with a CONFLICT:
# uninstall the Codex line first (install_aris_codex.sh --uninstall), then
# install the Kimi line. Codex-managed names the Kimi selection does not need
# are left alone; uninstall/reconcile never touch Codex-owned entries.
#
# Managed entries are tracked in:
#   <project>/.aris/installed-skills-kimi.txt      (project install, default)
#   ~/.aris/installed-skills-kimi.txt              (--global user install)
#
# Reviewer backends: in skills-kimi, review-heavy skills (auto-review-loop,
# paper-claim-audit, citation-audit, ...) use the Kimi-native subagent contract
# (kimi_subagent / kimi_subagent_continue) out of the box -- same-family,
# provisional. The cross-family upgrade path is registering
# mcp-servers/llm-chat (or an equivalent review channel) as described in the
# MCP registry in SETUP_GUIDE.md; see docs/KIMI_ADAPTATION.md.
#
# Usage:
#   bash tools/install_aris_kimi.sh [project_path] [options]
#   bash tools/install_aris_kimi.sh --global [options]
#
...
#
# Scope:
#   (default)        project-local install into <project>/.agents/skills/
#   --global         user-level install into ~/.kimi-code/skills/
#                    (manifest at ~/.aris/installed-skills-kimi.txt; no AGENTS.md block)
#
...
#   .aris/skills-declined-kimi.txt and never re-asked).
#
# Options:
...
#
# Safety rules enforced (same contract as install_aris_codex.sh):
...
#   S6  Concurrent runs in same scope serialize via mkdir lockdir.
...
#   S9  If .aris/, .agents/, .agents/skills/ (or ~/.kimi-code*) is itself a symlink, abort.
#   S13 Skill names must match ^[A-Za-z0-9][A-Za-z0-9._-]*$ (slug regex).
#   K1  Names listed in the Codex manifest hard-fail as CONFLICT (the two
#       lines have diverged and share one discovery path); this installer
#       never creates, updates, or removes Codex-owned entries.
#   K2  Legacy kimi manifests may still carry `shared-codex` rows from the
#       pre-divergence installer; uninstall keeps skipping them.

set -euo pipefail

MANIFEST_VERSION="1"
MANIFEST_NAME="installed-skills-kimi.txt"
MANIFEST_PREV_NAME="installed-skills-kimi.txt.prev"
DECLINED_NAME="skills-declined-kimi.txt"
CODEX_MANIFEST_NAME="installed-skills-codex.txt"
...
ARIS_DIR_NAME=".aris"
LOCK_DIR_NAME=".install-kimi.lock.d"
DOC_FILE_NAME="AGENTS.md"
BLOCK_BEGIN="<!-- ARIS-KIMI:BEGIN -->"
BLOCK_END="<!-- ARIS-KIMI:END -->"
SAFE_NAME_REGEX='^[A-Za-z0-9][A-Za-z0-9._-]*$'
BASE_PACKAGE="skills-kimi"
GLOBAL_SKILLS_DIR="$HOME/.kimi-code/skills"
...
if $GLOBAL_INSTALL && [[ -n "$PROJECT_PATH" ]]; then
    echo "Error: --global does not take a project path" >&2; exit 2
fi
    ...
    if [[ -n "$ARIS_REPO_OVERRIDE" ]]; then
        ...
        if [[ -d "$parent/skills/$BASE_PACKAGE" ]]; then
            ...
            for guess in \
                ...
                "$HOME/aris_repo" \
                "$HOME/.kimi-code/Auto-claude-code-research-in-sleep"; do
...
# Kimi installs the skills-kimi package only. The Codex packages
# (skills-codex / skills-codex-claude-review / skills-codex-gemini-review) and
# the skills-kimi package directory name itself are never scanned as skills.
selected_packages() {
    printf "%s\n" "$BASE_PACKAGE"
}
...
build_upstream_inventory() {
    ...
    tmp="$(mktemp -t aris-kimi-upstream-raw.XXXX)"
    : > "$out"
    : > "$tmp"
    ...
    # Keep the last entry for duplicate names, then sort by name for
    # deterministic plans.
    awk -F'|' '{row[$2]=$0} END {for (name in row) print row[name]}' "$tmp" | sort -t'|' -k2,2 > "$out"
    rm -f "$tmp"
}

# --- Selective install (#366, ported from install_aris_codex.sh) ---
# Upstream rows are 3-field (kind|name|source_rel); catalog skill names match
# the mainline skill names 1:1 (mirror directories share names with skills/<name>).
...
interactive_select() {  # $1 = upstream file, $2 = out (selected) file
    ...
    glist="$(mktemp -t aris-kimi-glist.XXXX)"
    while IFS=$'\t' read -r gid display desc; do
    ...
    local ungrouped; ungrouped="$(mktemp -t aris-kimi-ungrouped.XXXX)"
...
build_selection() {  # $1 = upstream file, $2 = declined-candidates out file, $3 = selected out file
    ...
    local excl; excl="$(mktemp -t aris-kimi-excl.XXXX)"
    ...
    if $fresh; then
        ...
        local new_file; new_file="$(mktemp -t aris-kimi-new.XXXX)"
    ...
    local pruned
    if [[ -s "$excl" ]]; then
        pruned="$(mktemp -t aris-kimi-pruned.XXXX)"
...
# --- Scope resolution (project default, --global for user level) ---
if $GLOBAL_INSTALL; then
    TARGET_ROOT="$HOME"
    SKILLS_REL=".kimi-code/skills"
    DOC_FILE=""
else
    ...
    DOC_FILE="$TARGET_ROOT/$DOC_FILE_NAME"
fi
ARIS_REPO="$(resolve_aris_repo)"
PROJECT_SKILLS_DIR="$TARGET_ROOT/$SKILLS_REL"
ARIS_STATE_DIR="$TARGET_ROOT/$ARIS_DIR_NAME"
MANIFEST_PATH="$ARIS_STATE_DIR/$MANIFEST_NAME"
MANIFEST_PREV="$ARIS_STATE_DIR/$MANIFEST_PREV_NAME"
LOCK_DIR="$ARIS_STATE_DIR/$LOCK_DIR_NAME"
LEGACY_NESTED="$TARGET_ROOT/.agents/skills/aris"
CODEX_MANIFEST_PATH="$ARIS_STATE_DIR/$CODEX_MANIFEST_NAME"
CATALOG_PATH="$ARIS_REPO/$CATALOG_REL"
DECLINED_PATH="$ARIS_STATE_DIR/$DECLINED_NAME"
...
check_no_symlinked_parents() {
    local p
    if $GLOBAL_INSTALL; then
        for p in "$ARIS_STATE_DIR" "$TARGET_ROOT/.kimi-code" "$PROJECT_SKILLS_DIR"; do
            if is_symlink "$p"; then
                die "$p is a symlink; refusing to mutate symlinked parent directories"
            fi
        done
    else
        for p in "$ARIS_STATE_DIR" "$TARGET_ROOT/.agents" "$PROJECT_SKILLS_DIR"; do
            if is_symlink "$p"; then
                die "$p is a symlink; refusing to mutate symlinked parent directories"
            fi
        done
...
check_legacy_nested_install() {
    ...
    if [[ -e "$LEGACY_NESTED" || -L "$LEGACY_NESTED" ]]; then
        die "legacy nested install detected at $LEGACY_NESTED. Remove or migrate it before using the flat .agents/skills/<name> layout."
    fi
}

write_lock_metadata() {
    cat > "$LOCK_DIR/owner.json" <<EOF
{"host":"$(hostname)","pid":$$,"started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","tool":"install_aris_kimi.sh"}
EOF
...
acquire_lock() {
    ...
    die "another install_aris_kimi.sh appears to be running (lock: $LOCK_DIR, owner: $owner)"
}

# Is this skill name already managed by install_aris_codex.sh in this project?
codex_manages() {  # $1 = name
    [[ -s "$CODEX_MANIFEST_DATA" ]] || return 1
    [[ -n "$(manifest_lookup_target "$CODEX_MANIFEST_DATA" "$1")" ]]
}

compute_plan() {
    local upstream_file="$1" manifest_data="$2" out="$3"
    local kind name source_rel target_path expected_target current_target in_manifest in_codex
    : > "$out"
    ...
    while IFS='|' read -r kind name source_rel; do
        ...
        in_codex=false
        codex_manages "$name" && in_codex=true

        # K1: the two release lines diverged and share one discovery path --
        # a name the Codex line manages can never be (co-)installed here.
        if $in_codex; then
            printf "CONFLICT|%s|%s|%s|codex_managed\n" "$kind" "$name" "$source_rel" >> "$out"
            continue
        fi
    ...
    while IFS=$'\t' read -r mkind mname msource mtarget mmode; do
        ...
        # Legacy shared-codex rows (pre-divergence installer) that left
        # upstream simply drop out of the new manifest; removal on disk is
        # the Codex installer's job (K2).
        [[ "$mmode" == "shared-codex" ]] && continue
...
write_manifest_tmp() {
    local plan="$1" out="$2"
    {
        ...
        printf "scope\t%s\n" "$($GLOBAL_INSTALL && echo global || echo project)"
        printf "installer\tinstall_aris_kimi.sh\n"
...
update_agents_doc() {
    ...
    $NO_DOC && return 0
    [[ -n "$DOC_FILE" ]] || return 0   # --global: no project AGENTS.md
    local original=""
    ...
    repo_lookup_cmd="ARIS_REPO=\$(awk -F'\\t' '\$1==\"repo_root\"{print \$2; exit}' \"$TARGET_ROOT/$ARIS_DIR_NAME/$MANIFEST_NAME\")"
    new_block="$BLOCK_BEGIN
## ARIS Kimi Code Skill Scope
ARIS skills installed in this project for Kimi Code CLI (source package: $BASE_PACKAGE).
...
One release line per project: the Kimi and Codex lines have diverged and share
this discovery path, so the Kimi installer refuses names already managed by
install_aris_codex.sh (CONFLICT). To switch lines, uninstall one before
...
\`bash $ARIS_REPO/tools/install_aris_kimi.sh \"$TARGET_ROOT\" --reconcile\`
Review-heavy skills use the Kimi-native subagent reviewer contract (same-family,
provisional); for a cross-family accepted route register mcp-servers/llm-chat
or an equivalent review channel (see docs/KIMI_ADAPTATION.md).
$BLOCK_END"
...
if len(matches) > 1:
    sys.stderr.write("ARIS-KIMI:WARN multiple managed blocks found; skipping update\n")
...
matches = pattern.findall(text)
if len(matches) > 1:
    sys.stderr.write("ARIS-KIMI:WARN multiple managed blocks found; skipping removal\n")
    sys.stdout.write(text)
else:
    updated = pattern.sub("\n", text)
...
do_uninstall() {
    ...
    local manifest_data
    manifest_data="$(mktemp -t aris-kimi-manifest.XXXX)"
    load_manifest "$MANIFEST_PATH" "$manifest_data"
    log ""
    log "Uninstall plan:"
    while IFS=$'\t' read -r kind name _source _target mode; do
        [[ -z "$name" ]] && continue
        if [[ "$mode" == "shared-codex" ]]; then
            log "  = $name ($kind, shared-codex — kept)"
        else
            log "  - $name ($kind)"
        fi
    done < "$manifest_data"
    ...
    while IFS=$'\t' read -r kind name source_rel target_rel mode; do
        [[ -z "$name" ]] && continue
        if [[ "$mode" == "shared-codex" ]]; then
            # K2: legacy pre-divergence rows; Codex-owned entries are never
            # removed by this installer.
            log "  = kept $name (managed by install_aris_codex.sh)"
            continue
        fi
...
print_review_backend_note() {
    $QUIET && return 0
    log ""
    log "Reviewer-backend note (Kimi Code):"
    log "  Review-heavy skills (auto-review-loop, paper-claim-audit, citation-audit, …)"
    log "  in skills-kimi use the Kimi-native subagent reviewer contract"
    log "  (kimi_subagent / kimi_subagent_continue) — same-family, provisional."
    log "  For a cross-family accepted route, register mcp-servers/llm-chat (or an"
    log "  equivalent review channel) — see SETUP_GUIDE.md §3.2 and docs/KIMI_ADAPTATION.md."
}

log ""
log "ARIS Kimi Code Install"
log "  Scope:     $($GLOBAL_INSTALL && echo "global ($HOME/.kimi-code/skills)" || echo "project")"
...
UPSTREAM_FILE="$(mktemp -t aris-kimi-upstream.XXXX)"
build_upstream_inventory "$ARIS_REPO" "$UPSTREAM_FILE"

MANIFEST_DATA="$(mktemp -t aris-kimi-manifest.XXXX)"
load_manifest "$MANIFEST_PATH" "$MANIFEST_DATA"

# Coexistence: load the Codex manifest (project scope only) so names it owns
# hard-fail as CONFLICT (K1) -- the two release lines diverged and share the
# .agents/skills/ discovery path, so one project runs exactly one line.
CODEX_MANIFEST_DATA="$(mktemp -t aris-kimi-codex-manifest.XXXX)"
if $GLOBAL_INSTALL; then
    : > "$CODEX_MANIFEST_DATA"
else
    load_manifest "$CODEX_MANIFEST_PATH" "$CODEX_MANIFEST_DATA"
fi

# Selective install (#366): build the selected set, then plan against it.
SELECTED_FILE="$(mktemp -t aris-kimi-selected.XXXX)"
DECLINED_CANDIDATES="$(mktemp -t aris-kimi-declined.XXXX)"
build_selection "$UPSTREAM_FILE" "$DECLINED_CANDIDATES" "$SELECTED_FILE"
require_office_author_for_selection "$SELECTED_FILE"
SELECTED_UPSTREAM="$(mktemp -t aris-kimi-upstream-sel.XXXX)"
...
PLAN_FILE="$(mktemp -t aris-kimi-plan.XXXX)"
compute_plan "$SELECTED_UPSTREAM" "$MANIFEST_DATA" "$PLAN_FILE"
print_plan "$PLAN_FILE"

if grep -q '^CONFLICT|' "$PLAN_FILE"; then
    if grep -q '|codex_managed$' "$PLAN_FILE"; then
        warn "this project already runs the Codex release line: the codex_managed names above are owned by install_aris_codex.sh (see $CODEX_MANIFEST_PATH)."
        warn "To switch lines, uninstall Codex first, then rerun this installer:"
        warn "  bash $ARIS_REPO/tools/install_aris_codex.sh \"$TARGET_ROOT\" --uninstall"
        warn "  bash $ARIS_REPO/tools/install_aris_kimi.sh \"$TARGET_ROOT\""
        warn "Or keep the Codex line as-is (do nothing)."
    fi
...
if $DRY_RUN; then
    ...
    rm -f "$UPSTREAM_FILE" "$MANIFEST_DATA" "$CODEX_MANIFEST_DATA" "$PLAN_FILE" "$SELECTED_FILE" "$SELECTED_UPSTREAM" "$DECLINED_CANDIDATES"
    exit 0
fi
...
INSTALLED_NAMES="$(mktemp -t aris-kimi-names.XXXX)"
...
if ! $DRY_RUN; then
    ...
    while IFS=$'\t' read -r kind name source_rel target_rel mode; do
        ...
        in_body && NF==5 { print }
    ' "$MANIFEST_PATH")
    (( local_bad == 0 )) && log "" && log "✓ Kimi Code install complete. $N_CHANGES changes applied."
    print_review_backend_note
fi

rm -f "$UPSTREAM_FILE" "$MANIFEST_DATA" "$CODEX_MANIFEST_DATA" "$PLAN_FILE" "$INSTALLED_NAMES" "$SELECTED_FILE" "$SELECTED_UPSTREAM" "$DECLINED_CANDIDATES"
