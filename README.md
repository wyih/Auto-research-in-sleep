...
# Optional Kimi Code install (project .agents/skills/, Kimi-native skills-kimi package;
# one release line per project — uninstall the Codex line first if it owns the project)
bash tools/install_aris_kimi.sh ~/your-kimi-project --office-author "Your Name"
# (user-level instead: bash tools/install_aris_kimi.sh --global)

# Managed Kimi Code project update
cd Auto-claude-code-research-in-sleep && git pull
...
**Want the Kimi Code install chain?** Use `tools/install_aris_kimi.sh` — project installs symlink the Kimi-native `skills/skills-kimi/` package into `.agents/skills/`, `--global` installs to `~/.kimi-code/skills/`; updates are one command via `tools/smart_update_kimi.sh --apply --project <path>`. The Kimi and Codex lines ship in **one unified release** (tags `<suite>-vX.Y.Z`; the notes give both install commands) and **one project runs exactly one line**: the installer refuses names already managed by `install_aris_codex.sh` — switch lines via `install_aris_codex.sh --uninstall` first. Review-heavy skills use the Kimi-native subagent contract (same-family / provisional); register `llm-chat` for a cross-family accepted route. See [docs/KIMI_ADAPTATION.md](docs/KIMI_ADAPTATION.md).
